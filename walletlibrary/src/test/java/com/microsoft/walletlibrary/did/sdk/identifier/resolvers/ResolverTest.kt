...
class ResolverTest {
    ...
    fun successfulResolutionTest() {
        val resolver = Resolver("", identifierService)
        coEvery { identifierService.resolveIdentifier("", expectedIdentifier) } returns KotlinResult.success(expectedIdentifierResponse)
        runBlocking {
    ...
    fun failedResolutionInvalidIdTest() {
        val resolver = Resolver("", identifierService)
        coEvery { identifierService.resolveIdentifier("", invalidIdentifier) } returns KotlinResult.failure(
            NotFoundException(
                "Not Found",
                true,
            )
    ...
    fun failedResolutionNetworkConnectionTest() {
        val resolver = Resolver("invalidUrl", identifierService)
        coEvery {
            identifierService.resolveIdentifier(
                "invalidUrl",
                expectedIdentifier
            )
