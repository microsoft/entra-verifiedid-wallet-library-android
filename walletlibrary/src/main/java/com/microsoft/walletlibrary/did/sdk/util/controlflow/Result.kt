...
internal suspend fun <T> runResultTry(operationName: String, block: suspend RunResultTryContext.() -> Result<T>): Result<T> =
    try {
        RunResultTryContext().block()
    } catch (ex: RunResultTryAbortion) {
        SdkLog.w("$operationName aborted", ex)
        Result.Failure(ex.error as SdkException)
    } catch (ex: CancellationException) {
        SdkLog.w("$operationName cancelled", ex)
        throw ex
    } catch (ex: SdkException) {
        SdkLog.w("$operationName Internal Sdk Exception", ex)
        Result.Failure(ex)
    } catch (ex: Exception) {
        SdkLog.e("$operationName Unhandled Sdk Exception", ex)
